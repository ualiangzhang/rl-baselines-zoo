...
class ExpertDataset(object):
    ...
    :param training_path: (str) The path to trajectory data (.npz file). Mutually exclusive with training_data.
    :param training_data: (dict) Trajectory data, in format described above. Mutually exclusive with training_path.
    ...
    def __init__(self, training_path=None, test_path=None, training_data=None, train_fraction=0.7, batch_size=64,
                 traj_limitation=-1, randomize=True, verbose=1, sequential_preprocessing=False):
        if training_data is not None and training_path is not None and test_path is not None:
            raise ValueError("Cannot specify both 'training_data' and 'training_path' and 'test_path'")
        if training_data is None and training_path is None or test_path is None:
            raise ValueError("Must specify one of 'training_data' or 'training_path' and 'test_path'")
        ...
        traj_limit_idx = len(training_data['obs'])
        ...
        observations_training = training_data['obs']
        actions_training = training_data['actions']

        observations_test = test_data['obs']
        actions_test = test_data['actions']
        ...
        if len(observations_training.shape) > 2:
            observations_training = np.reshape(observations_training, [-1, np.prod(observations_training.shape[1:])])
            observations_test = np.reshape(observations_test, [-1, np.prod(observations_test.shape[1:])])
        if len(actions_training.shape) > 2:
            actions_training = np.reshape(actions_training, [-1, np.prod(actions_training.shape[1:])])
            observations_test = np.reshape(observations_test, [-1, np.prod(observations_test.shape[1:])])

        indices_training = np.random.permutation(len(observations_training)).astype(np.int64)
        indices_test = np.random.permutation(len(observations_test)).astype(np.int64)

        # Train/Validation split when using behavior cloning
        train_indices = indices_training
        ...
        self.observations = observations_training
        self.actions = actions_training

        self.observations_test = observations_test
        self.actions_test = actions_test

        self.returns = training_data['episode_returns'][:traj_limit_idx]
        ...
        self.val_loader = DataLoader(val_indices, self.observations_test, self.actions_test, batch_size,
                                     shuffle=self.randomize, start_process=False,
                                     sequential=sequential_preprocessing)
